...
function Button({ children, onSelect, position }: { children: string; onSelect: any; position: any }) {
  ...
  return (
    <Interactive
      ...
      <Box args={[0.12, 0.05, 0.01]} position={position} attachArray={undefined} attachObject={undefined}>
        <meshPhongMaterial color={hover ? '#669' : '#666'} wireframe />
        <Text fontSize={0.03} position={[0, 0, 0.006]}  attachArray={undefined} attachObject={undefined}>
          {children}
        </Text>
      </Box>
    </Interactive>
  ...
  return (
    <>
      ...
      <ARCanvas>
        ...
        <OrbitControls attachArray={undefined} attachObject={undefined} enableKeys={undefined} />
        <Dots />
        <ambientLight />
        <DotsRender />
